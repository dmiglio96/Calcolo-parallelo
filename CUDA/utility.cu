...
int saveResultsOnFile(float time){
	...
	const char * wheretoprint = "resultsKNN_cudav2.out";
	...
	fprintf(fp, "Test with BLOCK_SIZE %d e K = %d trainingData %d x %d and testingData: %d x %d, time: %f\n\n", BLOCK_SIZE, K, N, M, P, M, time);
